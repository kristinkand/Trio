...
extension LiveActivitySettings {
    struct RootView: BaseView {
        ...
        @State private var shouldDisplayHintBubble: Bool = false
        ...
        var body: some View {
                    ...
                    if state.useLiveActivity {
                        ...
                        Section {
                            ...
                            if state.lockScreenView == .detailed {
                                ...
                                SettingInputSection(
                                    decimalValue: $decimalPlaceholder,
                                    booleanValue: $state.showBubbleInLiveActivity,
                                    shouldDisplayHint: $shouldDisplayHintBubble,
                                    selectedVerboseHint: Binding(
                                        get: { selectedVerboseHint },
                                        set: {
                                            selectedVerboseHint = $0.map { AnyView($0) }
                                            hintLabel = String(localized: "Show Glucose Bubble")
                                        }
                                    ),
                                    units: state.units,
                                    type: .boolean,
                                    label: String(localized: "Show Glucose Bubble"),
                                    miniHint: String(localized: "Display the round glucose bubble next to the chart."),
                                    verboseHint: VStack(alignment: .leading, spacing: 10) {
                                        Text("Default: ON").bold()
                                        Text(
                                            "When enabled, the Detailed Lock Screen widget shows a colored glucose bubble (matching the main app) next to the glucose chart. Turn this off to show just the chart."
                                        )
                                    },
                                    headerText: String(localized: "Show Glucose Bubble")
                                )
                            }
                        }.listRowBackground(Color.chart)
            ...
            .sheet(isPresented: $shouldDisplayHintBubble) {
                SettingInputHintView(
                    hintDetent: $hintDetent,
                    shouldDisplayHint: $shouldDisplayHintBubble,
                    hintLabel: hintLabel ?? "",
